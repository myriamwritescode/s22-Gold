...
for value_tag in span_tags_committee:
    if value_tag.contents[2].string is not None:
        if value_tag.contents[2].string.strip() != '':
            print(value_tag.sourceline, value_tag.contents[2].string.strip())
...
tags2_list = []
tags2 = soup.find_all('a', target="_blank")

for i in range(len(tags2)):
    matchvoice = re.search(r'Rep\.\s[a-zA-Z]+,.+\[', str(tags2[i].contents[0]))
    if matchvoice:
        snippet = matchvoice.group(0)
        tags2_list.append(snippet[5:len(snippet)-2])

for j in range(len(tags2_list)):
    if j % 2 == 0:
        print(tags2_list[j])
